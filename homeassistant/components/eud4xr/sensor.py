...
class ECAObject(ECAEntity):
    ...
    async def async_restores_original_settings(self) -> None:
        ...
        _LOGGER.info("Performed restores_original_settings action")
    ...
    async def async_hides(self) -> None:
        ...
        _LOGGER.info("Performed hides action")
    ...
    async def async_activates(self) -> None:
        ...
        _LOGGER.info("Performed activates action")
    ...
    async def async_deactivates(self) -> None:
        ...
        _LOGGER.info("Performed deactivates action")
...
class Switch(ECAEntity):
    ...
    async def async_toogle_status(self) -> None:
        ...
        _LOGGER.info("Performed toogle status action")
...
class ECAXRPointer(ECAEntity):
    ...
    def extra_state_attributes(self) -> dict:
        super_extra_attributes = super().extra_state_attributes
        return {"isPointed": self.isPointed, **super_extra_attributes}
...
class ECAXRInteractable(ECAEntity):
    ...
    def extra_state_attributes(self) -> dict:
        super_extra_attributes = super().extra_state_attributes
        return {"isInteracted": self.isInteracted, **super_extra_attributes}
...
class ECADoor_Custom(ECAEntity):
    """ECADoor_Custom defines the behavior of a door.

    Attributes:
    - None explicitly defined here, but it inherits from ECAEntity, so it inherits any attributes and methods from that class.

    """

    def __init__(self, **kwargs: dict) -> None:
        """Initializes an instance of the ECADoor_Custom class.

        Arguments:
        - kwargs: Any additional keyword arguments are passed to the parent class for initialization.

        """
        super().__init__(**kwargs)
        self._attr_should_poll = False  # Prevents polling of the state

    @property
    def extra_state_attributes(self) -> dict:
        """Returns extra state attributes for the ECADoor_Custom object.

        This method extends the attributes provided by the parent class to include custom door-related attributes.

        Returns:
        - dict: Extra attributes for the door object.

        """
        super_extra_attributes = super().extra_state_attributes
        return {**super_extra_attributes}

    @eca_script_action(verb="open")
    async def async_open(self) -> None:
        """Opens the door.

        This method is triggered when the door is opened.
        """
        _LOGGER.info("Performed opens action")

    @eca_script_action(verb="close")
    async def async_close(self) -> None:
        """Closes the door.

        This method is triggered when the door is closed.
        """
        _LOGGER.info("Performed closes action")

    @eca_script_action(verb="lock")
    async def async_lock(self) -> None:
        """Locks the door.

        This method is triggered when the door is locked.
        """
        _LOGGER.info("Performed locks action")

    @eca_script_action(verb="unlock")
    async def async_unlock(self) -> None:
        """Unlocks the door.

        This method is triggered when the door is unlocked.
        """
        _LOGGER.info("Performed unlocks action")


class ECABottle(ECAEntity):
    """ECABottle defines the behavior of a bottle.

    Attributes:
    - hass (HomeAssistant): A reference to the Home Assistant system to which the object belongs.
    - fillLevel (int): The current fill level of the bottle, represented as an integer (e.g., 0 to 100).
    - IsCapped (bool): Indicates whether the bottle is capped (True) or not (False).

    """

    def __init__(self, hass, fillLevel, IsCapped, **kwargs: dict) -> None:
        """Initialize an instance of the ECABottle class.

        Arguments:
        - hass: A reference to the Home Assistant system.
        - fillLevel: The fill level of the bottle (an integer value).
        - IsCapped: The state of the cap (True if capped, False otherwise).

        """
        super().__init__(hass=hass, **kwargs)
        self._attr_should_poll = False  # Prevents the state from being polled

        self._fillLevel = fillLevel  # Sets the fill level attribute
        self._isCapped = IsCapped  # Sets the capped state attribute

    @property
    def extra_state_attributes(self) -> dict:
        """Return extra state attributes of the ECABottle object.
        Extends the attributes provided by the parent class with custom attributes.

        Returns:
        - dict: Extra attributes for the ECABottle object, including fill level and cap state.

        """
        super_extra_attributes = super().extra_state_attributes or {}
        super_extra_attributes["fillLevel"] = self._fillLevel
        super_extra_attributes["isCapped"] = self._isCapped
        return super_extra_attributes
    ...
    async def async_fill(self, amount: int) -> None:
        """Fill the bottle by a specified amount.

        Arguments:
        - amount: The amount to increase the fill level by.

        """
        _LOGGER.info("Performed fill action")

    @eca_script_action(verb="decrease", variable="level", modifier="by")
    async def async_empty(self, amount: int) -> None:
        """Empty the bottle by a specified amount.

        Arguments:
        - amount: The amount to decrease the fill level by.

        """
        _LOGGER.info("Performed empty action")

    @eca_script_action(verb="open cap")
    async def async_open_cap(self) -> None:
        """Open the bottle cap."""
        _LOGGER.info("Performed open cap action")

    @eca_script_action(verb="close cap")
